...
type PersonalityValues = Pick<
  ProfileDetails,
  "personality" | "favorite_season" | "social_energy"
>;
type InterestsValues = Pick<
  ProfileDetails,
  "weekend_preferences" | "interests" | "hobbies"
>;
  ...
  return (
    <header className="onboarding-header">
      ...
      <div
        ...
        aria-valuemax={6}
        aria-valuenow={step}
        aria-valuetext={`Etapa ${step} de 6`}
      >
...
function ChoiceGroup({
  ...
  return (
    <fieldset className="onboarding-option-group m-0 min-w-0 border-0 p-0">
      <legend className="p-0 text-[12px] font-[660] text-[#3d3d3f]">
        {label}
      </legend>
      {children}
    </fieldset>
...
export function OnboardingFlow({ onFinished }: { onFinished?: () => void }) {
  ...
  async function finishOnboarding() {
    ...
    try {
      ...
      setError(
        "Não foi possível concluir seu perfil. Revise os dados obrigatórios.",
      );
      setSaving(false);
    }
  }
  ...
  if (step === 2)
    return (
      <LocationStep
        ...
        error={error}
        onBack={() => setStep(1)}
        onSubmit={async (values, improve) => {
          setSaving(true);
          setError(null);
          if (!values.stateCode || !values.city || !values.bio.trim()) {
            setError(
              "Escolha estado e cidade e escreva sua bio para continuar.",
            );
            setSaving(false);
            return;
          }
          try {
            ...
            setError("Não foi possível salvar sua localização e bio.");
          } finally {
            setSaving(false);
          }
        }}
...
function IdentityStep({
  ...
  const birthDateHintId = `${birthDateId}-hint`;
  return (
    <OnboardingPage step={1}>
      <StepHeading
        eyebrow="SUA IDENTIDADE"
        title="Como podemos chamar você?"
        description="Esses dados formam a base do seu perfil."
      />
      <form className="auth-form" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <ControlledInput
          ...
          rules={{
            required: "Escolha seu @username.",
            pattern: {
              value: /^@?[a-zA-Z0-9._]{3,30}$/,
              message: "Use 3 a 30 letras, números, ponto ou underline.",
            },
          }}
        />
        <Controller
          ...
          render={({ field, fieldState }) => (
            <div className="native-date-field">
              ...
              >
                {fieldState.error?.message ??
                  "No iPhone, o seletor nativo será aberto."}
              </small>
            </div>
          )}
        />
...
function LocationStep({
  ...
  error: string | null;
  onBack: () => void;
  onSubmit: (values: LocationForm, improve: boolean) => Promise<void>;
}) {
  ...
  async function changeState(nextState: string) {
    ...
    try {
      const nextCities = await getCitiesForState(nextState);
      if (cityRequestId.current !== requestId) return;
      setCities(nextCities);
    } catch {
      if (cityRequestId.current !== requestId) return;
      setCityError(true);
    } finally {
      if (cityRequestId.current === requestId) setLoadingCities(false);
    }
  }

  return (
    <OnboardingPage step={2} onBack={onBack}>
      <StepHeading
        eyebrow="SEU LUGAR"
        title="Onde sua história acontece?"
        description="A localização ajuda a encontrar pessoas e comunidades próximas."
      />
      <form className="auth-form" onSubmit={(event) => event.preventDefault()}>
        <div className="location-fields">
          <NativeSelect
            ...
            options={[
              ...
              ...brazilianStates.map(([value, label]) => ({ value, label })),
            ]}
          />
          <NativeSelect
            label="Cidade"
            ...
            disabled={!selectedState || loadingCities || cityError}
            aria-busy={loadingCities || undefined}
            hint={
              cityError
                ? "Não foi possível carregar as cidades. Escolha o estado novamente."
                : undefined
            }
            options={[
              {
        ...
        <Controller
          ...
          rules={{
            ...
            maxLength: { value: 300, message: "Use até 300 caracteres." },
          }}
          render={({ field, fieldState }) => (
            <TextArea
              label="Bio"
              ...
              hint={fieldState.error?.message ?? `${field.value.length}/300`}
              rows={4}
              className="orha-field"
              textAreaClassName="orha-textarea"
            />
          )}
        />
        <ControlledInput
        ...
        <button
          ...
          disabled={saving}
          onClick={() =>
            void form.handleSubmit((values) => onSubmit(values, false))()
          }
        >
          Concluir meu perfil agora
        </button>
      </form>
    </OnboardingPage>
  );
...
function PersonalityStep({
  ...
  onContinue,
  onSkip,
}: OptionalStepProps<PersonalityValues>) {
  const [personality, setPersonality] = useState<string[]>(
    details.personality ?? [],
  );
  const [season, setSeason] = useState(details.favorite_season ?? "");
  const [energy, setEnergy] = useState(details.social_energy ?? "");

  return (
    <OnboardingPage step={3} onBack={onBack}>
      <StepHeading
        eyebrow="SEU JEITO"
        title="Como você se define?"
        description="Escolha até cinco características. Você poderá mudar isso depois."
      />
      <ChoiceGroup label="Personalidade">
        <ChoiceChips
      ...
      <ChoiceGroup label="Estação preferida">
        <ChoiceChips
          options={seasonOptions}
          values={season ? [season] : []}
          onChange={(values) => setSeason(values.at(-1) ?? "")}
          max={1}
        />
      </ChoiceGroup>
      <ChoiceGroup label="Energia social">
        <ChoiceChips
          options={energyOptions}
          values={energy ? [energy] : []}
          onChange={(values) => setEnergy(values.at(-1) ?? "")}
          max={1}
        />
      </ChoiceGroup>
      {error ? (
...
function InterestsStep({
  ...
  onContinue,
  onSkip,
}: OptionalStepProps<InterestsValues>) {
  const [weekend, setWeekend] = useState<string[]>(
    details.weekend_preferences ?? [],
  );
  const [interests, setInterests] = useState<string[]>(details.interests ?? []);
  const [hobbies, setHobbies] = useState<string[]>(details.hobbies ?? []);

  return (
    <OnboardingPage step={4} onBack={onBack}>
      <StepHeading
        eyebrow="O QUE MOVE VOCÊ"
        title="Do que você gosta?"
        description="Usaremos isso para aproximar você de pessoas e conversas."
      />
      <ChoiceGroup label="No fim de semana">
        <ChoiceChips
      ...
      <div className="onboarding-actions">
        <Button
          ...
          isLoading={saving}
          onPress={() =>
            onContinue({ weekend_preferences: weekend, interests, hobbies })
          }
        >
          Continuar
        </Button>
...
function PlacesStep({
  ...
  onContinue,
  onSkip,
}: OptionalStepProps<PlacesValues>) {
  const [visited, setVisited] = useState(
    (details.visited_places ?? []).join(", "),
  );
  const [desired, setDesired] = useState(
    (details.desired_places ?? []).join(", "),
  );
  const visitedId = useId();
  const visitedHintId = `${visitedId}-hint`;
  ...
  const parse = (value: string) =>
    value
      ...
      .slice(0, 20);
  return (
    <OnboardingPage step={5} onBack={onBack}>
      <StepHeading
        eyebrow="SEU MUNDO"
        title="Por onde você já passou?"
        description="Viagens também criam conversas e afinidades."
      />
      <div className="auth-form">
        <div className="plain-field">
          <label htmlFor={visitedId}>
            <MapPin size={15} aria-hidden="true" /> Lugares que já visitou
          </label>
          <textarea
            id={visitedId}
            value={visited}
            onChange={(event) => setVisited(event.target.value)}
            placeholder="Ex.: Salvador, Lisboa, Gramado"
            rows={3}
            aria-describedby={visitedHintId}
          />
          <small id={visitedHintId}>Separe os lugares por vírgulas.</small>
        </div>
        <div className="plain-field">
          <label htmlFor={desiredId}>
            <Sparkles size={15} aria-hidden="true" /> Lugares que deseja
            conhecer
          </label>
          <textarea
            id={desiredId}
            ...
            placeholder="Ex.: Roma, Jerusalém, Patagônia"
            rows={3}
          />
        </div>
        {error ? (
        ...
        <Button
          ...
          onPress={() =>
            onContinue({
              visited_places: parse(visited),
              desired_places: parse(desired),
            })
          }
        >
          Continuar
