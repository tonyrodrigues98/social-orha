...
export const NativeSelect = ({
  ...
  const reactId = useId();
  const selectId = providedId ?? `select-native-${reactId}`;
  const labelId = `${selectId}-label`;
  const hintId = `${selectId}-hint`;
  const describedBy =
    [providedDescription, hint ? hintId : undefined]
      .filter(Boolean)
      .join(" ") || undefined;
    ...
    <div className={cx("w-full in-data-input-wrapper:w-max", className)}>
      ...
      <div className="relative grid w-full items-center">
        <select
          ...
          aria-labelledby={providedLabel ?? (label ? labelId : undefined)}
          className={cx(
      ...
      {hint && (
        <HintText className="mt-2" id={hintId} aria-live="polite">
          {hint}
        </HintText>
      )}
    </div>
