...
export function validateProfileMediaSelection(
  ...
  if (files.length === 0) {
    return { ok: false, code: "empty", message: "Nenhuma imagem foi selecionada." };
  }
  if (availableSlots <= 0) {
    return {
      ok: false,
  ...
  if (files.some((file) => !supportedProfileImageTypes.has(file.type))) {
    return { ok: false, code: "type", message: "Use imagens JPEG, PNG, WebP ou AVIF." };
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      if (
        ...
        reject(new Error("A imagem selecionada excede 12.000 pixels de largura ou altura."));
        return;
      }
      resolve(dimensions);
    };
