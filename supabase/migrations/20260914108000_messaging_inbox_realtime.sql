...
begin
  if to_regclass('realtime.messages') is null
    ...
    raise exception 'Messaging inbox Realtime prerequisites are missing.';
  end if;
end;
$$;
...
begin
  if not exists (
    ...
    where schemaname = 'realtime'
      ...
      and qual like '%messaging-inbox:%'
      and qual like '%auth.uid()%'
  ) then
    raise exception 'Private messaging inbox Realtime channel policy was not installed.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'Realtime publication is missing public.conversations.';
  end if;
end;
$$;
