...
Commit-base publicado antes deste checkpoint: `28cae8b`
...
Estado: **staging saudável e alinhado às 22 migrations locais; 40 tabelas públicas com RLS, 16 tabelas Realtime, cinco Edge Functions ativas, gates anônimo/CORS 7/7 e autenticado catálogo/export 8/8, dois jobs Cron/Vault ativos; Preview Vercel `Ready` porém ainda protegido por SSO; Auth remoto sincronizado; onboarding atômico; suporte persistente, gestão global de papéis RPC-only e consentimento de analytics owner-only; produção intacta**.
...
O gate autenticado staging passou 8/8 com conta efêmera e cleanup verificado: sessão por senha, catálogo externo real, request persistida de exportação, criação/download privado, integridade SHA-256 e owner, retry idempotente e remoção do objeto/usuário. O script recusa explicitamente o project-ref de produção e não imprime credenciais nem signed URLs. Mídia autenticada, lifecycle destrutivo e purge real permanecem gates separados.
...
- concluído nesta fatia: `npm run smoke:edge:authenticated` passou 8/8 contra staging com conta efêmera removida; catálogo real, export privado, SHA-256/owner e retry idempotente foram comprovados sem resíduos nem exposição de secrets.
- gates atuais: matriz E2E pública completa `111/111`, inclusive WebKit 390×844 após reforço dos alvos de toque canônicos contra arredondamento subpixel; guards/deep links de `/suporte` e `/admin/funcoes` revalidados em `66/66` nos 11 projetos públicos; Playwright descobre 128 execuções em 16 arquivos — 111 públicas, 12 jornadas autenticadas e cinco setups de sessão —; Vitest `352/352` em 80 arquivos, Deno `11/11`, Edge anônimo/CORS `7/7`, Edge autenticado catálogo/export `8/8`, catálogo, TypeScript, ESLint, secrets, encoding, dívida de produção, auditoria de dependências, orçamento de bundle e build PWA com 103 entradas aprovados;
