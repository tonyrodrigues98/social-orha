...
Commit-base publicado antes deste checkpoint: `b771d32`
...
Estado: **staging saudável e alinhado às 28 migrations locais; 40 tabelas públicas com RLS, 17 tabelas Realtime, cinco Edge Functions ativas, gates anônimo/CORS 7/7, autenticado catálogo/export/mídia 14/14, ciclo de conta 17/17, mensageria/mídia privada 18/18, domínio social 20/20, navegador social efêmero 3/3, matriz de cinco papéis 16/16, WebKit privado 6/6, foco de teclado 6/6 e baseline remoto curto 6/6, dois jobs Cron ativos; Preview Vercel `Ready` porém ainda protegido por SSO; Auth remoto sincronizado; produção intacta**.

## Resumo executivo

O banco de produção ainda não recebeu a cadeia social desta branch. No staging isolado `bgeauxljwjbtbwpbzpoo`, as 28 migrations versionadas, de `20260811040000` a `20260914110000`, estão agora aplicadas e registradas no ledger remoto. A consulta atual do ledger confirmou paridade local/remota e `supabase db lint --linked` retornou `No schema errors found` depois da migration mais recente. O gate estrutural passou em 23/23 checks e as matrizes transacionais de RLS/onboarding/suporte/papéis globais/consentimento passaram integralmente, terminando em `ROLLBACK` sem fixtures residuais.
...
- concluído: 28 migrations ledgered, `supabase db lint --linked` sem achados, validação estrutural 23/23 e matrizes RLS/Storage/Realtime/onboarding/suporte/papéis globais/consentimento com rollback;
...
- todas as 28 migrations locais estão aplicadas no staging; produção permanece sem promoção;
