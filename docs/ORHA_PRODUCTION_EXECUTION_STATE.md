...
Commit-base publicado antes deste checkpoint: `f482bc4`
...
Estado: **staging saudável e alinhado às 27 migrations locais; 40 tabelas públicas com RLS, 17 tabelas Realtime, cinco Edge Functions ativas, gates anônimo/CORS 7/7, autenticado catálogo/export/mídia 14/14, ciclo de conta 17/17, mensageria/mídia privada 18/18, domínio social 20/20 e navegador social efêmero 3/3, dois jobs Cron ativos; Preview Vercel `Ready` porém ainda protegido por SSO; Auth remoto sincronizado; produção intacta**.

## Resumo executivo

O banco de produção ainda não recebeu a cadeia social desta branch. No staging isolado `bgeauxljwjbtbwpbzpoo`, as 27 migrations versionadas, de `20260811040000` a `20260914109000`, estão agora aplicadas e registradas no ledger remoto. O lint hospedado passa sem erros nem avisos, o gate estrutural passou em 23/23 checks e as matrizes transacionais de RLS/onboarding/suporte/papéis globais/consentimento passaram integralmente, terminando em `ROLLBACK` sem fixtures residuais.
...
- concluído: 27 migrations ledgered, lint SQL remoto sem achados, validação estrutural 23/23 e matrizes RLS/Storage/Realtime/onboarding/suporte/papéis globais/consentimento com rollback;
...
- pendente: contas sintéticas permanentes A/B/Admin/Moderador/Suporte e execução das jornadas autenticadas completas já contratadas, inclusive Auth por e-mail, suporte, autoridade por papel e rejeição da senha atual incorreta; amizade, comunidade e conversa de texto já possuem prova adicional efêmera 3/3 no navegador;
...
- todas as 27 migrations locais estão aplicadas no staging; produção permanece sem promoção;
