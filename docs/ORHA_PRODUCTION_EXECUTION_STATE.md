...
Commit-base publicado antes deste checkpoint: `9f76d7d`
...
Estado: **staging saudável e alinhado às 29 migrations locais; 40 tabelas públicas com RLS, 17 tabelas Realtime, cinco Edge Functions ativas, gates anônimo/CORS 7/7, autenticado catálogo/export/mídia 14/14, ciclo de conta 17/17, mensageria/mídia privada 18/18, domínio social 20/20, navegador social efêmero 3/3, matriz de cinco papéis 16/16, mensageria avançada 6/6, WebKit privado 6/6, foco de teclado 6/6 e baseline remoto curto 6/6, dois jobs Cron ativos; Preview Vercel `Ready` porém ainda protegido por SSO; Auth remoto sincronizado; produção intacta**.

## Resumo executivo

O banco de produção ainda não recebeu a cadeia social desta branch. No staging isolado `bgeauxljwjbtbwpbzpoo`, as 29 migrations versionadas, de `20260811040000` a `20260914112000`, estão agora aplicadas e registradas no ledger remoto. A consulta atual do ledger confirmou paridade local/remota e `supabase db lint --linked` retornou `No schema errors found` depois da migration autoritativa mais recente. O gate estrutural passou em 23/23 checks e as matrizes transacionais de RLS/onboarding/suporte/papéis globais/consentimento passaram integralmente, terminando em `ROLLBACK` sem fixtures residuais.
...
O gate `npm run smoke:social:domain` passou 20/20 usando os mesmos princípios de isolamento. Duas contas efêmeras com onboarding concluído comprovaram descoberta server-side, negação da identidade de mensageria antes de vínculo, liberação mínima após pedido consentido, negação de autoamizade, pedido/aceite/notificações, deduplicação do par, bloqueio de escrita privilegiada direta, criação de comunidade com owner derivado no servidor, descoberta/entrada/saída/reentrada, autoridade exclusiva de manager, publicação, comentário, reações e notificações persistentes, bloqueio global removendo e impedindo amizade, desbloqueio restaurando contato consentido e negação de leitura social ao papel `anon`. O `finally` removeu a comunidade e os dois usuários; a pós-validação confirmou zero Auth users, perfis e comunidades do smoke.

O gate `npm run test:e2e:staging:social` passou 3/3 em Chromium 390×844 usando duas sessões independentes e contas efêmeras confirmadas. Pela interface pública real, comprovou login, descoberta, pedido/aceite/remoção de amizade, criação e entrada em comunidade, publicação, reação e comentário recuperados após reload, além de solicitação consentida de conversa, atualização da caixa sem reload, aceite, deep link, mensagem entregue por Realtime e persistência após reload. O gate encontrou e corrigiu invalidações incompletas, dois embeds PostgREST incompatíveis com o schema, projeção indevida de perfis crus, heading duplicado, seletores ambíguos, falso positivo de request cancelada e o contrato de dismiss durante mutation; o cleanup confirmou zero perfis, conversas e comunidades efêmeros residuais.

O gate `npm run test:e2e:staging:matrix` passou 16/16 em Chromium 390×844 com cinco identidades efêmeras de papéis distintos. Ele comprovou setup e autoridade exata de Usuário A, Usuário B, Admin, Moderador e Suporte; navegação autenticada; retomada offline; perfil, upload e privacidade; troca e restauração de senha, logout, troca de conta e deep link; comunidade; amizade; conversa consentida com texto, imagem, WAV, waveform, recibo e preferência; isolamento de sessão; suporte; bloqueio, denúncia e moderação. A execução focada de mensageria passou `6/6` e acrescentou reply, reação pelo RPC `set_message_reaction`, encaminhamento e soft-delete por ID estável; a migration `20260914112000` removeu DML direto da tabela para o navegador e deriva o ator de `auth.uid()`. Cada contexto multiusuário cria uma sessão Supabase independente, evitando reuso de refresh token rotativo; o cleanup final removeu e verificou as cinco identidades e seus dados.
...
- `20260914108000_messaging_inbox_realtime.sql`: aplicado; autoriza somente `messaging-inbox:<auth.uid()>`, publica `conversations` e mantém a caixa de entrada reconciliada sem reload; SHA-256 `0A6905A642261CCEC87D48A9F94645698AA44E377F6E064E3FB4A8A495B3642A`.
- `20260914109000_messaging_profile_summaries.sql`: aplicado; RPC limitada a 100 perfis e autorizada somente por pedido ou membership, sem reabrir leitura crua de `profiles`; SHA-256 `2A5174AE4FCE9D9AF278DF94A8509FD9FA6F08D6297456DCAB2E9482FDC1F7FD`.
- `20260914110000_own_blocked_profile_summaries.sql`: aplicado; substitui leitura crua de perfis bloqueados por projeção owner-only `SECURITY DEFINER`, com limite e menor privilégio; SHA-256 `8746E932A84BC82B798F3E6988A3CF1E9B55053068915C1DE4E9039751188F77`.
- `20260914112000_authoritative_message_reactions.sql`: aplicado; reação idempotente RPC-only, ator derivado de Auth, validações de conta/membership/watermark/bloqueio e DML direto revogado de `authenticated`; SHA-256 `DF6DBFCC95F37029275F419D4CAE862D2C42EE85BBF1349D7CCEEDC0C7E85548`.
...
| Workers/lifecycle        | `20260816180000_edge_worker_contracts.sql` + cinco Edge Functions   | mesma implementação versionada                                                      | catálogo/export/mídia 14/14; ciclo de conta 17/17; mensageria 18/18 e social 20/20; matriz permanente de cinco papéis pendente |
...
| Realtime                 | 17 tabelas + canais privados de conversa/inbox/notificações/suporte | mesma configuração                                                                  | aplicado; pedido e entrega de texto sem reload e readiness de replicação comprovados entre dois usuários efêmeros              |
...
- Postgres Changes: 17 tabelas — amizades, bloqueios, memberships/conteúdo comunitário, conversas/members/requests/preferences/messages/reactions/attachments/receipts, notificações, tickets e mensagens de suporte;
...
- concluído: 29 migrations ledgered, `supabase db lint --linked` sem achados depois da migration autoritativa mais recente, validação estrutural 23/23 e matrizes RLS/Storage/Realtime/onboarding/suporte/papéis globais/consentimento com rollback;
...
- concluído nesta fatia: templates Auth em português para confirmação, recuperação e alerta de senha alterada foram versionados e cobertos por 4/4 testes; a tentativa real de `config push` chegou ao Auth remoto e foi rejeitada atomicamente porque o projeto Free ainda usa o mailer padrão. O Supabase exige custom SMTP ou upgrade para permitir alteração de templates, portanto o remoto preserva os templates padrão até esse bloqueador externo ser resolvido.
- concluído nesta fatia: a matriz private native-first passou `15/15`, incluindo cinco setups reais e dez viewports (`320×568`, `375×667`, `390×844`, `393×852`, `430×932`, `440×932`, tablet retrato/paisagem, `1280×800`, `1440×900`). Cada viewport percorreu as cinco áreas com WCAG A/AA, campos de 16 px, alvos de 44 px, overflow, console/network e screenshot. O gate encontrou e corrigiu contraste 1,08:1 no CTA de Conversas e 1,06:1 no estado vazio do Perfil, ambos causados por conflito entre tokens ORHA/Untitled e shadcn.
- concluído nesta fatia: o gate privado WebKit/iPhone 390×844 passou `6/6` (cinco setups reais mais a jornada WebKit), percorreu Início, Comunidade, Explorar, Conversas e Perfil com sessão Supabase renovada, `prefers-reduced-motion: reduce`, WCAG A/AA, inspeção de console/network e screenshot; todas as identidades efêmeras foram removidas ao final.
- concluído nesta fatia: a expiração de sessão passou `6/6` no Auth real. O gate cria uma sessão Supabase válida, revoga remotamente o refresh token, marca o access token armazenado como expirado para obrigar o refresh, comprova a rejeição `400/401`, o redirecionamento seguro para `/auth/login?redirect=/inicio`, a nova autenticação e a persistência após reload; nenhuma identidade efêmera permaneceu no staging.
- concluído nesta fatia: rede móvel lenta e mídia privada indisponível passaram em dois gates `6/6`. O Chromium DevTools aplicou 650 ms de latência e throughput limitado sem interceptar requests; o app preservou o launch state e abriu Início/Notificações com a11y e runtime limpos. Uma linha efêmera `profile_media` pronta, mas sem objeto no Storage, comprovou o fallback “Imagem indisponível”, retry acessível e ausência de imagem quebrada; a linha e as cinco identidades foram removidas e verificadas.
- concluído nesta fatia: baseline remoto curto passou `6/6` no staging com cinco identidades simultâneas e cleanup verificado. Foram cinco logins e 90 leituras sociais reais: Auth p95/p99 385/385 ms; Home 60/110 ms; perfis 54/56 ms; interesses 50/64 ms; todos com zero erros. O gate registra as métricas no relatório Playwright e no log sanitizado. Ele não é apresentado como teste de capacidade: envelope de lançamento, execução sustentada por 60 minutos a 3× o pico e backpressure permanecem pendentes.
- concluído nesta fatia: o Drawer canônico GodUI/React Aria passou `6/6` no staging. O gate navegou somente por teclado, manteve 24 sequências de Tab/Shift+Tab dentro dos Drawers de edição e privacidade, fechou por Escape e confirmou a devolução de foco aos acionadores originais; as cinco identidades efêmeras foram removidas e verificadas.
- concluído nesta fatia: a jornada real de mensageria passou novamente `6/6` ao contrair o viewport focado de 390×844 para 390×520. Header e composer permaneceram visíveis, sem colisão, o textarea conservou foco e fonte computada de 16 px, e texto, imagem, WAV/waveform, recibo e preferência continuaram persistentes; o runner removeu e verificou as cinco identidades. Isso não substitui teclado nativo em iPhone físico.
- concluído nesta fatia: a jornada avançada de mensageria passou `6/6` no staging com reply, reação, encaminhamento e soft-delete além de texto, imagem, WAV/waveform, receipt e preferências. O teste revelou um `403` real na gravação direta de `message_reactions`; a migration `20260914112000` substituiu-a pelo RPC actor-authoritative, revogou DML direto do navegador e foi confirmada no ledger remoto. Cinco identidades e os dados efêmeros foram removidos e verificados.
...
- concluído nesta fatia: Deno 2.8 verificou os cinco entrypoints de Edge e executou `11/11` testes. O gate revelou e corrigiu o tipo de bytes do SHA-256 para `ArrayBuffer` compatível com Web Crypto e converteu o teste do parser de lifecycle para o runner Deno real. `check:edge` e `test:edge` passaram a integrar o workflow obrigatório.
...
- pendente: custom SMTP + domínio de envio, CAPTCHA e prova de entrega real de confirmação/reenvio/reset; os templates e callbacks já estão versionados, mas o Auth remoto não aceita publicá-los com o mailer padrão do plano Free;
- concluído: runner de matriz E2E cria cinco contas efêmeras A/B/Admin/Moderador/Suporte, valida papéis reais, executa 16/16 jornadas autenticadas sem interceptações ou mocks e remove todas as identidades ao final; ainda faltam confirmação/reenvio/reset por e-mail real depois do SMTP;
...
- pendente: fechar a QA privada de teclado aberto e PWA em iPhone físico; WebKit autenticado emulado, sessão expirada real, rede lenta e mídia quebrada estão concluídos. Depois: domínio definitivo, PostHog/dashboards/alertas, carga sustentada/backpressure sobre envelope aprovado, escala humana e promoção controlada da produção.
...
- todas as 29 migrations locais estão aplicadas no staging; produção permanece sem promoção;
