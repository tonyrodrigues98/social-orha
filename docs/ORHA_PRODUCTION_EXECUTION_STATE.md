...
Commit-base publicado antes deste checkpoint: `ada660f`
...
- concluído nesta fatia: a conta Vercel autorizada foi conectada ao GitHub e o projeto `social-orha` foi criado sem Production Deployment. As variáveis detectadas foram substituídas por URL/chave publicável do staging e base `/`, todas limitadas a Preview; o build foi sobrescrito para `npm run build:vercel`. A referência `codex/production-launch@ada660f` está validada no formulário de Preview, aguardando apenas a confirmação de publicação exigida no clique final.
- gates atuais: matriz E2E pública completa `111/111`, inclusive WebKit 390×844 após reforço dos alvos de toque canônicos contra arredondamento subpixel; guards/deep links de `/suporte` e `/admin/funcoes` revalidados em `66/66` nos 11 projetos públicos; Playwright descobre 128 execuções em 16 arquivos — 111 públicas, 12 jornadas autenticadas e cinco setups de sessão —; Vitest `349/349` em 79 arquivos e Deno `11/11`, catálogo, TypeScript, ESLint, secrets, encoding, dívida de produção, auditoria de dependências, orçamento de bundle e build PWA com 103 entradas aprovados;
...
- pendente: confirmar/criar o Preview `codex/production-launch`, provar root/deep links por HTTP 200 e somente depois provisionar Production com os valores públicos aprovados;
