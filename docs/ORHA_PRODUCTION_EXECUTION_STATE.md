...
- concluído nesta fatia: o gate remoto de abuso foi reconciliado com o onboarding atômico atual e passou no staging; ele comprovou `PT429`, rollback da mutação excedente, contadores limitados, auditoria sem conteúdo/target, bloqueio antes do consumo de quota e isolamento das tabelas/funções privadas. A leitura posterior confirmou zero resíduos em Auth, perfis, contadores e auditoria.
- gates atuais: matriz E2E pública completa `111/111`, inclusive WebKit 390×844 após reforço dos alvos de toque canônicos contra arredondamento subpixel; guards/deep links de `/suporte` e `/admin/funcoes` revalidados em `66/66` nos 11 projetos públicos; Playwright descobre 128 execuções em 16 arquivos — 111 públicas, 12 jornadas autenticadas e cinco setups de sessão —; Vitest, catálogo, TypeScript, ESLint, secrets, encoding, dívida de produção, auditoria de dependências, orçamento de bundle e build PWA com 103 entradas aprovados;
