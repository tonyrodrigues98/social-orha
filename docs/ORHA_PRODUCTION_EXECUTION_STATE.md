...
- gates atuais: matriz E2E pública completa `111/111`, inclusive WebKit 390×844 após reforço dos alvos de toque canônicos contra arredondamento subpixel; guards/deep links de `/suporte` e `/admin/funcoes` revalidados em `66/66` nos 11 projetos públicos; Vitest `318/318` em 74 arquivos, catálogo, TypeScript, ESLint, secrets, encoding, dívida de produção, auditoria de dependências e build PWA com 103 entradas aprovados;
...
- pendente: mídia real, Realtime multiusuário, QA native-first/PWA, host definitivo, provisionamento do PostHog/alertas/SLO e promoção controlada da produção.
